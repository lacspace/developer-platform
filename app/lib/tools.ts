...
await serveStdio(server);` },
  ...
  },
  {
    ...
    features: [
      ...
      { icon: "🔁", title: "Retry with backoff", desc: "--retry N recovers flaky endpoints (network errors + 408/429/5xx) with exponential backoff + jitter." },
      { icon: "⏱️", title: "Latency benchmark", desc: "--repeat N fires a request N times and reports min/mean/median/p95/p99/max." },
      { icon: "📼", title: "HAR 1.2 export", desc: "--har out.har writes a standard log you can open in Chrome DevTools, Insomnia or Postman." },
      { icon: "🧪", title: "# @expect assertions", desc: "A friendlier alias for # @assert, with json.<path> shorthand for JSON-body checks." },
    ],
    examples: [
      ...
      { label: "Retry a flaky endpoint", code: `lacspace-http https://api.example.com/flaky --retry 3 --retry-delay 500`, note: "Exponential backoff + jitter on network errors and 5xx." },
      { label: "Benchmark latency", code: `lacspace-http https://httpbin.org/get --repeat 50`, note: "Prints min/mean/median/p95/p99/max; add --json for scripts." },
      { label: "Export a HAR log", code: `lacspace-http run api.http --har run.har`, note: "Open the run in Chrome DevTools or any HAR viewer." },
    ],
    useCases: [
    ...
    keywords: ["http", "http-client", "rest-client", "api-client", "curl", "httpie", "postman", "http-file", "api-testing", "ci", "retry", "backoff", "har", "benchmark", "latency", "p95", "response-assertions"],
  },
  {
    slug: "fake",
    name: "lacspace-fake",
    tagline: "Deterministic fake data → JSON/CSV/SQL, now relational.",
    icon: "🎲",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🔗", title: "Linked tables with FKs", desc: "Generate users then orders where userId: ref(users.id) — every foreign key is guaranteed to exist." },
      { icon: "🧩", title: "Template fields", desc: "Compose values like full:template({{firstName}} {{lastName}}) from earlier fields or generators." },
      { icon: "🆔", title: "Unique constraints", desc: "unique(uuid) guarantees no duplicate values across rows." },
      { icon: "📐", title: "CREATE TABLE DDL", desc: "--ddl emits schema with column types inferred from the data." },
      { icon: "🌐", title: "Spanish & French locales", desc: "Realistic names, addresses and phones in es/fr (plus en/ne)." },
      { icon: "🎰", title: "15+ new generators", desc: "ULID, hex/rgb/hsl colour, semver, MIME type, file paths, timezone, IBAN/BIC, masked credit card." },
    ],
    examples: [
      ...
      { label: "Linked tables → SQL with DDL", code: `npx lacspace-fake --relations shop.json -f sql --ddl`, note: "Foreign keys resolve to real parent ids; CREATE TABLE types inferred." },
      { label: "Template + unique fields", code: `npx lacspace-fake --fields "id:unique(uuid),first:firstName,handle:template({{first}}-{{int(1..99)}})" -n 5`, note: "No duplicate ids; handle interpolates the row's own fields." },
      { label: "French locale", code: `npx lacspace-fake --fields "name:fullName,phone:phone" --locale fr -n 5`, note: "French names and +33 mobiles, deterministic under --seed." },
      { label: "New generators", code: `npx lacspace-fake ulid -n 5 --seed 42`, note: "Also iban, semver, hexColor, mimeType, filePath, timezone." },
    ],
    useCases: [
    ...
    keywords: ["fake-data", "seed-data", "mock-data", "test-fixtures", "faker", "sql-insert", "csv", "deterministic", "nepal", "database-seeding", "ulid", "ddl", "foreign-keys", "relations", "locale", "template", "unique", "iban", "semver"],
  },
  {
    slug: "deps",
    name: "lacspace-deps",
    tagline: "Dependency, licence & SBOM auditor as a CI gate.",
    icon: "📦",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "📋", title: "SBOM export", desc: "Generate a CycloneDX 1.5 or SPDX 2.3 JSON bill of materials for your whole dependency tree." },
      { icon: "📜", title: ".depsrc.json policy", desc: "Keep allow/deny, max-severity and fail-on gates in the repo; auto-discovered, CLI overrides." },
      { icon: "🚦", title: "Max-severity licence gate", desc: "Fail on anything stricter than a chosen category, no glob lists needed." },
      { icon: "🔖", title: "Package-URL (purl) ids", desc: "Every SBOM component carries a standard pkg:npm/... purl and bom-ref." },
    ],
    examples: [
      ...
      { label: "CycloneDX SBOM", code: `npx lacspace-deps sbom --format cyclonedx > bom.json`, note: "Standards-compliant CycloneDX 1.5 JSON." },
      { label: "SPDX SBOM", code: `npx lacspace-deps sbom ./my-app --format spdx > sbom.spdx.json`, note: "SPDX 2.3 with purl externalRefs + DESCRIBES relationships." },
      { label: "Severity gate in CI", code: `npx lacspace-deps --max-severity weak-copyleft --fail-on license`, note: "Exit 1 on any strong-copyleft or unknown licence." },
      { label: "Committed policy", code: `npx lacspace-deps --policy ./.depsrc.json`, note: "Allow/deny + maxSeverity + failOn from one JSON file." },
    ],
    useCases: [
    ...
    keywords: ["dependencies", "dependency-audit", "license-checker", "spdx", "supply-chain", "unused-dependencies", "depcheck", "outdated", "install-size", "ci", "cyclonedx", "sbom", "license-policy", "depsrc", "purl", "compliance"],
  },
  {
    slug: "json",
    name: "lacspace-json",
    tagline: "The friendly jq — query, patch, convert, canonicalize.",
    icon: "🧾",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🛣️", title: "JSONPath queries", desc: "$.store.book[*].price, recursive $..author, wildcards, slices and [?(@.price<10)] filters." },
      { icon: "🩹", title: "JSON Patch (RFC 6902)", desc: "Apply a patch or generate one from two documents; add/remove/replace/move/copy/test." },
      { icon: "📍", title: "JSON Pointer (RFC 6901)", desc: "Resolve /a/b/0 addresses from CLI or library, with escape-aware parsing." },
      { icon: "🪜", title: "flatten / unflatten", desc: "Collapse to a { \"a.b[0]\": v } map and rebuild it, round-trip lossless." },
      { icon: "🔏", title: "Canonical JSON", desc: "Deep key-sort + minimal whitespace for stable hashing and reproducible diffs." },
    ],
    examples: [
      ...
      { label: "JSONPath recursive", code: `echo '{"store":{"book":[{"price":8.95},{"price":22.5}]}}' | npx lacspace-json -q '$..price'`, note: "$-style JSONPath with recursive descent." },
      { label: "Generate a JSON Patch", code: `npx lacspace-json diff old.json new.json --patch`, note: "RFC 6902 patch that turns old into new." },
      { label: "Apply a JSON Patch", code: `npx lacspace-json patch doc.json changes.json`, note: "add/remove/replace/move/copy/test ops." },
      { label: "Canonical JSON", code: `npx lacspace-json sort data.json --canonical | shasum`, note: "Sorted keys, minimal whitespace for stable hashing." },
    ],
    useCases: [
    ...
    keywords: ["jq", "json", "yaml", "toml", "csv", "json-schema", "json-diff", "json-merge", "jsonpath", "convert", "json-patch", "rfc6902", "json-pointer", "flatten", "canonicalize", "sort-keys"],
  },
  {
    slug: "qr",
    name: "lacspace-qr",
    tagline: "Styled, branded QR codes — SVG, PNG, ANSI.",
    icon: "🔳",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🔲", title: "Styled modules", desc: "Square, dots or rounded module shapes, with independently styled finder eyes." },
      { icon: "🪪", title: "Centre logo / initials", desc: "Embed a logo, data: URI or initials with an auto-cleared quiet band (ECC-guarded)." },
      { icon: "🌈", title: "Gradient foreground", desc: "Two-tone linear-gradient fill via a real SVG <linearGradient>." },
      { icon: "🖥️", title: "ANSI terminal render", desc: "Crisp full-block QR, one module per cell, scannable off the screen." },
      { icon: "📅", title: "Calendar QR", desc: "Build an iCalendar VEVENT for events (timed or all-day)." },
      { icon: "🗄️", title: "Batch subcommand", desc: "One QR file per CSV/TXT row, named from a column." },
    ],
    examples: [
      ...
      { label: "Dot modules + rounded eyes", code: `npx lacspace-qr "https://lacspace.com" -f svg -o qr.svg --shape dots --eye rounded`, note: "Branded module styling, still fully scannable." },
      { label: "Gradient foreground", code: `npx lacspace-qr "https://lacspace.com" -f svg -o qr.svg --gradient "#4d9fff,#a855f7,45"`, note: "Emits a real SVG <linearGradient>." },
      { label: "ANSI terminal", code: `npx lacspace-qr "https://lacspace.com" --ansi`, note: "Full-block render, one module per cell." },
      { label: "Batch from CSV", code: `npx lacspace-qr batch urls.csv --out out/ -f png --scale 8`, note: "One PNG per row, named by the id column." },
    ],
    useCases: [
    ...
    keywords: ["qr", "qrcode", "qr-generator", "reed-solomon", "svg", "png", "wifi-qr", "vcard", "barcode", "offline", "ansi", "styled-qr", "qr-logo", "gradient", "calendar", "ics", "batch"],
  },
  {
    slug: "svg",
    name: "lacspace-svg",
    tagline: "Optimize & componentize SVGs — React/Vue/Svelte/Solid.",
    icon: "✏️",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🖼️", title: "Vue / Svelte / Solid", desc: "One toComponent dispatcher plus vue/svelte/solid CLI commands, alongside React." },
      { icon: "🎨", title: "currentColor injection", desc: "Swap literal fill/stroke for currentColor so icons inherit CSS color, with a keep-list." },
      { icon: "✨", title: "Smarter optimizer", desc: "Drops default-valued presentation attrs, hoists shared attributes, strips Sketch/Illustrator cruft." },
      { icon: "📐", title: "viewBox & dimension fixing", desc: "Derive a missing viewBox from width/height (and back), plus a dimensions() reader." },
      { icon: "📊", title: "Batch stats", desc: "Optimize a whole folder and get per-file and total bytes saved, human or --json." },
    ],
    examples: [
      ...
      { label: "Vue 3 SFC", code: `npx lacspace-svg vue logo.svg --name Logo --ts`, note: "Also svelte, solid, or component --framework <f>." },
      { label: "Inherit CSS color", code: `npx lacspace-svg optimize "icons/*.svg" --current-color --keep-colors "#fff"`, note: "fill/stroke become currentColor; kept colours stay literal." },
      { label: "Batch + totals", code: `npx lacspace-svg optimize "icons/*.svg" --precision 2`, note: "Prints per-file and a total bytes-saved summary." },
      { label: "Fix a missing viewBox", code: `npx lacspace-svg optimize icon.svg --fix-dimensions`, note: "Derives viewBox from width/height and vice-versa." },
    ],
    useCases: [
    ...
    keywords: ["svg", "svgo", "svg-optimizer", "svg-to-jsx", "svg-to-react", "data-uri", "svg-sprite", "svg-parser", "svg-minify", "react-svg", "svg-to-vue", "svg-to-svelte", "svg-to-solid", "currentcolor", "svg-viewbox"],
  },
  {
    slug: "i18n",
    name: "lacspace-i18n",
    tagline: "Locale linter — ICU, .po convert, auto-sync, coverage.",
    icon: "🌍",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🎯", title: "ICU MessageFormat validation", desc: "Parses plural/select/number/date messages and flags malformed structure, argument mismatches and missing 'other' branches." },
      { icon: "🔄", title: "gettext .po conversion", desc: "Read/write .po alongside JSON, YAML and .properties, and convert between any pair." },
      { icon: "🔁", title: "Auto-sync translations", desc: "Fill each locale with the base's missing keys (marker or base value) without overwriting existing translations." },
      { icon: "🚦", title: "Coverage CI gate", desc: "--min N exits non-zero when a locale falls below a threshold; fill markers excluded from the count." },
    ],
    examples: [
      ...
      { label: "Convert gettext .po to JSON", code: `npx lacspace-i18n convert messages.po -o en.json`, note: "Formats inferred from extensions; --from/--to override." },
      { label: "Auto-sync locales to the base", code: `npx lacspace-i18n sync ./locales --from-base --write`, note: "Fills missing keys from base values; existing translations untouched." },
      { label: "Deep ICU check", code: `npx lacspace-i18n icu ./locales`, note: "Catches plurals missing 'other' and dropped arguments a plain diff misses." },
      { label: "Gate CI on coverage", code: `npx lacspace-i18n coverage ./locales --min 90`, note: "Exits 1 if any locale is below 90% (markers excluded)." },
    ],
    useCases: [
    ...
    keywords: ["i18n", "internationalization", "localization", "translation", "missing-keys", "icu", "icu-messageformat", "i18next", "vue-i18n", "react-intl", "gettext", "po", "messageformat", "plural", "format-conversion", "auto-sync"],
  },
  {
    slug: "changelog",
    name: "lacspace-changelog",
    tagline: "Conventional Commits → CHANGELOG + semver bump.",
    icon: "📝",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "👥", title: "Contributors section", desc: "Aggregate unique authors + co-authors, de-duped and sorted, with --contributors." },
      { icon: "⚙️", title: "Custom type config", desc: "A .changelogrc.json remaps types to sections and controls which bump the version / stay hidden." },
      { icon: "⚠️", title: "Breaking-change highlighting", desc: "A dedicated BREAKING CHANGES block from both type!: and BREAKING CHANGE:, forcing a major." },
      { icon: "🔀", title: "Compare & issue links", desc: "GitHub/GitLab/Bitbucket compare URLs plus #123 and commit-SHA linkifying." },
      { icon: "♻️", title: "Duplicate-safe merge", desc: "Re-runs won't double-prepend a version already in CHANGELOG.md." },
      { icon: "📜", title: "Git-range generate", desc: "generate --from v1.0.0 --to HEAD reads the log directly, no piping." },
    ],
    examples: [
      ...
      { label: "Notes with contributors", code: `npx lacspace-changelog notes --from v1.0.0 --contributors`, note: "Release body with a Contributors list." },
      { label: "Generate for a range", code: `npx lacspace-changelog generate --from v1.2.0 --to HEAD`, note: "Reads git log directly." },
      { label: "Custom type config", code: `npx lacspace-changelog --config .changelogrc.json`, note: "type→section/bump/hidden mapping honoured by grouping and bump." },
    ],
    useCases: [
    ...
    keywords: ["changelog", "conventional-commits", "semver", "release-notes", "keep-a-changelog", "version-bump", "changelog-generator", "release-please", "standard-version", "ci", "contributors", "breaking-changes", "compare-links", "changelogrc", "git-log"],
  },
  {
    slug: "schema",
    name: "lacspace-schema",
    tagline: "Infer schema, gen TS & Zod, validate, OpenAPI.",
    icon: "🧬",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "✅", title: "Validate against a schema", desc: "{ valid, errors[] } in the lib; non-zero CLI exit for CI gates." },
      { icon: "🛡️", title: "Zod codegen", desc: "Turn JSON or a schema into Zod source with z.infer types." },
      { icon: "🔁", title: "OpenAPI round-trip", desc: "Extract components.schemas/definitions and wrap schemas back into a components block or full doc." },
      { icon: "🔗", title: "Broader $ref resolution", desc: "Local #/definitions, #/$defs and #/components/schemas across every generator." },
      { icon: "🧩", title: "More formats", desc: "Adds time and ipv6 detection (on top of uuid/date-time/date/email/uri/ipv4)." },
    ],
    examples: [
      ...
      { label: "Validate a payload (CI gate)", code: `npx lacspace-schema validate user.schema.json user.json`, note: "Prints per-field errors and exits 1 when invalid." },
      { label: "JSON → Zod", code: `echo '{"id":1,"email":"a@x.com"}' | npx lacspace-schema zod --name User`, note: "Emits z.object({...}) + export type User = z.infer<typeof User>." },
      { label: "Extract OpenAPI components", code: `npx lacspace-schema openapi api.json`, note: "Pulls components.schemas (or Swagger 2 definitions) into { name: schema }." },
    ],
    useCases: [
    ...
    keywords: ["json-schema", "schema-inference", "json-to-typescript", "typescript-types", "codegen", "draft-07", "quicktype", "schema-diff", "ndjson", "type-inference", "json-schema-validator", "zod", "json-to-zod", "openapi", "swagger", "$ref"],
  },
  {
    slug: "size",
    name: "lacspace-size",
    tagline: "Bundle size — gzip/brotli, budgets, treemap, PR table.",
    icon: "📏",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🧬", title: "Composition breakdown", desc: "See where a bundle's bytes go: code vs strings vs comments vs whitespace, largest literals, module split." },
      { icon: "🗺️", title: "Treemap JSON", desc: "--breakdown --json emits a nested, treemap-friendly structure ready to visualize." },
      { icon: "💬", title: "PR-comment table", desc: "-f comment prints a tight Markdown table with a signed Δ-vs-baseline column and a Total row." },
      { icon: "📋", title: "Summary line", desc: "--summary collapses the whole run into one CI-friendly status line." },
      { icon: "🚦", title: "Named regression gate", desc: "--fail-over <size|%> fails a PR when the bundle grows past a threshold." },
      { icon: "⚙️", title: "Auto .sizerc.json", desc: "Commit your size policy once; it's picked up with no --config flag." },
    ],
    examples: [
      ...
      { label: "Break a bundle down", code: `npx lacspace-size dist/bundle.js --breakdown`, note: "Code/strings/comments/whitespace split + largest strings + module banners." },
      { label: "PR-comment table", code: `npx lacspace-size dist --baseline .size.json -f comment`, note: "Markdown table with a signed Δ column, ready to paste into a PR." },
      { label: "One-line summary", code: `npx lacspace-size dist --summary`, note: "files · raw · gzip · brotli · budgets · Δ — great as a CI step name." },
      { label: "Regression gate", code: `npx lacspace-size dist --baseline .size.json --fail-over 5%`, note: "Exit non-zero if the total grew more than 5%." },
    ],
    useCases: [
    ...
    keywords: ["bundle-size", "size-limit", "gzip-size", "brotli-size", "bundle-analyzer", "size-budget", "size-diff", "baseline", "ci", "regression", "treemap", "bundle-composition", "pr-comment", "sizerc", "fail-over"],
  },
  {
    slug: "mock",
    name: "lacspace-mock",
    tagline: "Local mock REST/GraphQL from JSON or OpenAPI.",
    icon: "🎭",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "📘", title: "Mock from OpenAPI 3", desc: "Generate routes + example responses from any spec's component schemas." },
      { icon: "🛂", title: "Request validation", desc: "Reject malformed bodies/params with a 400 + details (per-route or per-collection)." },
      { icon: "🎲", title: "Chaos status pools", desc: "Fail a % of responses with any statuses you choose (429,503,…), not just 500." },
      { icon: "📼", title: "Record & replay proxy", desc: "Forward unknown routes to a real upstream, capture responses, replay them offline." },
      { icon: "💾", title: "Stateful CRUD + reset", desc: "Mutations persist for the session; reset() snaps the db back for clean test runs." },
    ],
    examples: [
      ...
      { label: "Mock an OpenAPI spec", code: `npx lacspace-mock --openapi openapi.json`, note: "One route per operation, example bodies from your schemas." },
      { label: "Chaos with a status pool", code: `npx lacspace-mock db.json --error-rate 0.2 --error-statuses 429,503`, note: "20% of responses fail with a 429 or 503." },
      { label: "Record then replay offline", code: `npx lacspace-mock --db db.json --proxy https://api.example.com --record --recordings tape.json`, note: "Re-run with --replay to serve the cassette with no network." },
    ],
    useCases: [
    ...
    keywords: ["mock", "mock-server", "mock-api", "json-server", "fake-api", "api-mocking", "graphql", "crud", "prototyping", "fixtures", "openapi", "record-replay", "request-validation", "chaos-engineering", "api-simulator"],
  },
  {
    slug: "license",
    name: "lacspace-license",
    tagline: "LICENSE + headers + detection + compatibility.",
    icon: "⚖️",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.2.0",
    ...
    features: [
      ...
      { icon: "🔍", title: "Licence detection", desc: "Identify the SPDX id of any LICENSE file with a confidence score, tolerant of whitespace/CRLF and preamble." },
      { icon: "🔀", title: "Compatibility checker", desc: "Flag dependency licences that clash with your project (e.g. GPL in an MIT project) via a built-in matrix." },
      { icon: "🔧", title: "Auto-repair (--fix)", desc: "Write a missing LICENSE, align package.json's license field, and insert missing headers in one command." },
      { icon: "⚖️", title: "Copyleft-aware verdicts", desc: "Weak copyleft (LGPL/MPL) is marked \"review\", strong copyleft is failed, permissive always passes." },
    ],
    examples: [
      ...
      { label: "Detect a licence", code: `npx lacspace-license detect LICENSE`, note: "Prints the SPDX id + confidence; --json for CI." },
      { label: "Check dependency compatibility", code: `npx lacspace-license compat --id MIT --deps Apache-2.0,GPL-3.0-only`, note: "Non-zero exit when a dep licence is incompatible." },
      { label: "Auto-fix a failing gate", code: `npx lacspace-license check --require-headers --fix`, note: "Writes LICENSE, aligns package.json, adds headers." },
    ],
    useCases: [
  ...
  },
  {
    slug: "excel",
    name: "lacspace-excel",
    tagline: "The Excel Swiss-army knife — convert anything ⇄ Excel, run formulas, prebuilt templates.",
    icon: "📗",
    grad: "#4d9fff,#4d9fff",
    status: "live",
    version: "0.1.0",
    summary:
      "A keyless CLI + typed library for spreadsheets. Convert JSON, CSV, TSV, NDJSON, YAML, TOML, Markdown, HTML and SQL to real .xlsx and back, inspect any workbook, add computed columns with a safe Excel-style formula engine, de-duplicate, split and merge sheets — and generate 17 ready-to-fill templates (invoice, inventory, payroll, cashbook, loan schedule…) with live Excel formulas inside. Zero dependencies beyond the Lacspace Sheets Kit.",
    about:
    ...
    install: "npx lacspace-excel convert data.json -o data.xlsx",
    quickstart: "npm i lacspace-excel",
    tryHref: "/tools/excel/try",
    features: [
      { icon: "🔄", title: "Any ⇄ Excel", desc: "JSON, CSV, TSV, NDJSON, YAML, TOML, Markdown, HTML and SQL to .xlsx — and back — with input auto-detection." },
      { icon: "🪜", title: "Nested JSON → columns", desc: "address.city, items.0.sku flatten to columns and unflatten back, losslessly." },
      { icon: "🧠", title: "Typed inference", desc: "Numbers, booleans and dates are recognised only when a whole column agrees; IDs and zip codes stay text." },
      { icon: "🔍", title: "Inspect a workbook", desc: "Sheets, dimensions, inferred column types and sample rows — as a table or --json." },
      { icon: "𝑓", title: "Computed columns", desc: "--add \"margin=(price-cost)/price*100\" — 110+ Excel-style functions, SUMIFS, LOOKUP, dates, text; never eval." },
      { icon: "📑", title: "17 prebuilt templates", desc: "Invoice, quotation, purchase order, inventory, price list, CRM, orders, expenses, payroll, cashbook, attendance, budget, tax register, timesheet, project tracker, loan/EMI schedule, grade book." },
      { icon: "🧮", title: "Live formulas inside", desc: "Templates carry real =B2*C2 / SUM() cells with cached values — Excel, Numbers and Sheets recalculate them." },
      { icon: "🧹", title: "Dedupe, split, merge", desc: "Drop duplicate rows by column, split a workbook into per-sheet files, or merge files into one workbook." },
      { icon: "🛡️", title: "Injection-safe output", desc: "Cells starting with = + - @ are neutralised in CSV; SQL values are properly quoted." },
      { icon: "📚", title: "Function reference", desc: "lacspace-excel functions SUMIF — signature, description and example for every function." },
      { icon: "🚰", title: "Pipe-friendly", desc: "Reads stdin, writes stdout for text formats, exits non-zero on failure — drop it into CI or a cron job." },
      { icon: "🔒", title: "Zero deps, fully typed", desc: "Dual ESM+CJS library sharing the CLI engine; runs in Node, edge and the browser." },
    ],
    examples: [
      { label: "JSON → Excel", code: `npx lacspace-excel convert orders.json -o orders.xlsx --flatten`, note: "Nested objects become columns; one sheet per top-level key." },
      { label: "Excel → JSON", code: `npx lacspace-excel convert book.xlsx --sheet Orders --to json`, note: "Typed rows to stdout; pick a sheet by name or index." },
      { label: "CSV → Markdown table", code: `cat people.csv | npx lacspace-excel convert --from csv --to markdown --columns name,email`, note: "Reads stdin; keeps only the columns you want." },
      { label: "Inspect a workbook", code: `npx lacspace-excel inspect sales.xlsx`, note: "Sheets, rows × columns, inferred types, sample rows." },
      { label: "Add a computed column", code: `npx lacspace-excel formula stock.xlsx --add "margin=(price-cost)/price*100" -o stock.xlsx`, note: "Safe formula engine; the column is written with values." },
      { label: "Generate a template", code: `npx lacspace-excel template invoice -o invoice.xlsx`, note: "Ready to fill, with live totals. `template list` shows all 17." },
      { label: "Dedupe by column", code: `npx lacspace-excel dedupe leads.csv --by email -o clean.csv`, note: "Keeps the first occurrence." },
      { label: "Use it as a library", code: `import { convert } from "@lacspace/convert";\nimport { buildTemplate } from "lacspace-excel";\n\nconst xlsx = await convert(rows, { to: "xlsx", sheet: "Orders" });\nconst invoice = buildTemplate("invoice");` },
    ],
    useCases: [
    ...
    links: [
      { label: "npm", href: "https://www.npmjs.com/package/lacspace-excel", external: true },
      { label: "Source", href: "https://github.com/lacspace/npm-packages/tree/main/lacspace-excel", external: true },
      { label: "@lacspace/formula", href: "/packages/formula" },
      { label: "@lacspace/convert", href: "/packages/convert" },
    ],
    keywords: ["excel", "xlsx", "json-to-excel", "excel-to-json", "csv-to-xlsx", "xlsx-to-csv", "json-to-csv", "spreadsheet", "converter", "excel-templates", "invoice-template", "inventory-template", "payroll-template", "excel-formulas", "sumif", "vlookup", "cli"],
  },
];
