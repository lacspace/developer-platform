    ...
    items: [
      {
        ...
        a: "It's a free ecosystem for JavaScript and TypeScript developers: a library of more than 85 zero-dependency `@lacspace` packages, a set of standalone command-line tools, and `create-lacspace-app` — a CLI that scaffolds a finished Next.js app. Everything is documented at developer.lacspace.com and published openly to npm.",
      },
      {
    ...
    items: [
      {
        ...
        a: "There are more than 85 packages, grouped into kits: Core, Security, SEO, React, App & Utils, Backend, Data, Commerce & Ledger, Stock, Mail, Web, the AI Kit and regional payments. Between them they cover authentication, JWTs, crypto, validation, forms, SEO metadata, sitemaps, Open Graph images, money, dates, CSV/Excel, caching, rate-limiting, LLM chat and streaming, prompt engineering, token counting, RAG chunking and much more.",
      },
      {
    ...
    items: [
      {
        ...
        a: "Twenty-four free, keyless tools: lacspace-leads (Google Maps lead finder), lacspace-scraper (website scraper), lacspace-monitor (change & uptime monitor), lacspace-enrich (company & contact enrichment), lacspace-extract (PDF/DOCX/HTML extraction), lacspace-sql (SQL over data files), lacspace-inspect (site audit), lacspace-cron (cron toolkit), lacspace-dotenv (.env toolkit), lacspace-webhook (webhook receiver), lacspace-har (HAR analyser), lacspace-icon (favicon/PWA generator), lacspace-http (terminal API client & .http runner), lacspace-fake (fake/seed data generator), lacspace-deps (dependency & licence auditor), lacspace-json (jq-style query, convert, validate & merge), lacspace-qr (QR-code generator), lacspace-svg (SVG optimizer & converter), lacspace-i18n (translation-file linter), lacspace-changelog (Conventional-Commits changelog & semver), lacspace-schema (JSON→Schema→TypeScript codegen), lacspace-size (bundle-size budgets), lacspace-mock (local mock REST/GraphQL server) and lacspace-license (SPDX LICENSE & header manager).",
      },
      {
