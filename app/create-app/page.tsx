...
const seo = site.page({
  ...
  path: "/create-app",
  description:
    "create-lacspace-app scaffolds a finished Next.js 15 app in ~0.12s — 9 production-ready templates, SEO, security headers, sitemap, robots and a dynamic OG image wired from the first render, plus a catalog of 18 composable add-ons: a 96-component UI kit, charts/tables/date pickers, AI chat, chat-with-your-docs RAG, content, search, auth, analytics, payments, email, i18n, quality gates, uploads, web push, keyless CAPTCHA, in-app toasts, live SSE updates and GDPR cookie consent — added with --with, keyless and free-first, and the backend ones auto-upgrade you to full-stack. Not a blank page: a finished one.",
});
...
const ADDONS = [
  { icon: "🎛️", key: "ui", name: "ui", d: "The 96-component @lacspace/components library, already wearing your template's accent and following its dark-mode toggle — plus a working settings page and a one-file theme bridge." },
  { icon: "📊", key: "dataviz", name: "dataviz", d: "A working /insights page: stat cards, an SVG line chart, a date-range filter and a sortable, searchable, CSV-exportable data table. No canvas, no D3, no grid licence." },
  ...
  { icon: "🔔", key: "push", name: "push", d: "Web push notifications with zero vendors — VAPID + RFC 8291 over pure Web Crypto, a subscribe endpoint and a service worker. No Firebase/FCM.", fs: true },
  { icon: "🛡️", key: "captcha", name: "captcha", d: "A keyless, privacy-friendly proof-of-work CAPTCHA — stateless HMAC-signed challenges verified with one server secret. No Google, no Cloudflare, no tracking.", fs: true },
  { icon: "🍞", key: "notify", name: "notify", d: "Beautiful in-app toast notifications — a <Toaster/> and useToast wired in, with promise toasts, actions and positions. Accessible, zero-dependency." },
  { icon: "📡", key: "realtime", name: "realtime", d: "Live updates over Server-Sent Events — a channel/room hub, a Web-standard stream endpoint and a typed browser client with a useSSE hook. No WebSocket server, no vendor.", fs: true },
  { icon: "🍪", key: "consent", name: "consent", d: "GDPR-friendly cookie consent — a framework-agnostic store, a drop-in banner and a React <ConsentBanner/> + useConsent, cookie-persisted with whenConsent() script-gating." },
];

const STEPS = [
  { n: "01", t: "Run one command", d: "npm create lacspace-app@latest — no config files, no setup wizard." },
  { n: "02", t: "Pick a template + add-ons", d: "Nine templates. Choose static (frontend only) or --fullstack — frontend plus a real Node/Express/MongoDB/Redis backend. Then layer on any of 18 add-ons with --with ui,dataviz,ai-chat,rag,content,search,auth-pages,analytics,payments,email,i18n,quality,uploads,push,captcha,notify,realtime,consent — AI, content, search, auth, analytics, payments, email, i18n, quality gates, uploads, web push, keyless CAPTCHA, in-app toasts, live SSE updates and GDPR cookie consent, all keyless and free-first. The backend ones auto-upgrade a static project to full-stack." },
...
const CMP = [
  ...
  { m: "Prebuilt add-on catalog", a: "✗", b: "18: UI components, charts/tables/dates, AI, auth, payments, email, analytics, search, content, i18n, quality, uploads, push, captcha, toasts, realtime, consent", hot: true },
...
const FAQ = [
  ...
  { q: "What add-ons are included?", a: "Eighteen, each requested with --with <key>, in the interactive picker, or add <key> later: ui (the 96-component @lacspace/components library wired into your template — a working settings page, a <UIProvider>, and a theme bridge that maps your template\u2019s accent, radius and font onto the kit so the components match your brand and follow the dark-mode toggle you already have), dataviz (a working /insights page built from @lacspace/charts, @lacspace/table and @lacspace/date — stat cards, an SVG line chart, a date-range filter and a sortable, searchable, CSV-exportable table), ai-chat (a streaming AI chat on local Ollama), rag (chat with your Markdown docs, grounded with sources), content (a /updates Markdown section + RSS + llms.txt), search (instant keyless BM25 full-text search over your Markdown — no key, no service), auth-pages (profile editing, password change and TOTP 2FA on the built-in login), analytics (privacy-first, cookieless web analytics with a MongoDB collector and dashboard), payments (checkout wired to eSewa & Khalti with integer-safe money), email (transactional email with ready templates that logs to the console until you add SMTP), i18n (a dependency-free t() translator and language switcher with English + Nepali locales), quality (one-command quality gates — bundle-size budget, dependency audit and fake fixtures — plus a GitHub Actions CI workflow), uploads (authenticated file uploads stored in MongoDB, served via signed, expiring URLs — no S3), push (web push notifications with zero vendors — VAPID + RFC 8291 over pure Web Crypto, a subscribe endpoint and a service worker, no Firebase/FCM), captcha (a keyless, privacy-friendly proof-of-work CAPTCHA — stateless HMAC-signed challenges verified with one server secret, no Google or Cloudflare), notify (beautiful in-app toast notifications — a <Toaster/> and useToast with promise toasts, actions and positions), realtime (live updates over Server-Sent Events — a channel/room hub, a Web-standard stream endpoint and a typed browser client with a useSSE hook, no WebSocket server or vendor) and consent (GDPR-friendly cookie consent — a framework-agnostic store, a drop-in banner and a React <ConsentBanner/> + useConsent, cookie-persisted with whenConsent() script-gating). Everything is keyless and free-first; the backend ones — auth-pages, analytics, payments, email, uploads, push, captcha and realtime — contribute real Express routes and models via a route manifest and auto-upgrade a static project to full-stack, while ui, dataviz, i18n, quality, notify and consent are frontend/tooling add-ons." },
    ...
    <>
      ...
      <main className="wrap">
        ...
        <section className="hero">
          <Reveal><span className="pill"><span className="live" /> Open source · 9 templates · 18 add-ons · full-stack mode · 0.12s scaffold</span></Reveal>
          <Reveal delay={60}>
...
real  0.12s   ·   run 2: 0.12s   run 3: 0.12s   run 4: 0.12s   run 5: 0.12s`} />
        ...
        <section className="sec">
          <Reveal className="sec-head center">
            ...
            <h2>Eighteen add-ons, <span className="grad">one flag each</span></h2>
